<?php
declare(strict_types=1);

namespace App\Features\Operasi\SkorAldrette;
use App\Core\Controller\ControllerTemplate;
use App\Core\Controller\InputType as I;

final class SkorAldretteController extends ControllerTemplate
{
    public function __construct()
    {
        parent::__construct(
            model: new SkorAldretteModel(),
            breadcrumbs: [
                ['title' => 'Operasi', 'icon' => 'operasi'],
                ['title' => 'Skor Aldrette', 'icon' => 'skor_aldrette'],
            ],
            judul: 'Skor Aldrette',
            aksi: [
                'tambah' => true,
                'audit'  => true,
                'ubah'   => true,
                'hapus'  => true,
            ],
            konfig: [
                //visible, display, kolom, jenis, required
                [HIDE, 'ID Skor Aldrette',   'id_skor_aldrette',   I::INDEX,  OPTIONAL],
                [SHOW, 'Nomor Registrasi',    'nomor_reg',          I::TEXT,    REQUIRED],
                [SHOW, 'Waktu Penilaian',     'waktu_penilaian',    I::DATE, REQUIRED],
                [SHOW, 'ID Petugas',          'id_petugas',         I::TEXT,    REQUIRED],
                [SHOW, 'Dokter Anestesi',     'id_dokter_anestesi', I::TEXT,    REQUIRED],
                [SHOW, 'Skor Aktivitas',      'skor_aktivitas',     I::NUMBER,  REQUIRED],
                [SHOW, 'Skor Respirasi',      'skor_respirasi',     I::NUMBER,  REQUIRED],
                [SHOW, 'Skor Tekanan Darah',  'skor_tekanan_darah', I::NUMBER,  REQUIRED],
                [SHOW, 'Skor Kesadaran',      'skor_kesadaran',     I::NUMBER,  REQUIRED],
                [SHOW, 'Skor Warna Kulit',    'skor_warna_kulit',   I::NUMBER,  REQUIRED],
                [SHOW, 'Total Skor',          'total_skor',         I::NUMBER,  REQUIRED],
                [SHOW, 'Boleh Pindah',        'is_boleh_pindah',    I::SELECT,  REQUIRED],
                [SHOW, 'Catatan Keluar',      'catatan_keluar',     I::TEXT,    REQUIRED],
                [SHOW, 'Instruksi RR',        'instruksi_rr',       I::TEXT,    REQUIRED],
            ],
        );
    }
}