        ...
        <form action="<?= $modul_path . $form_action ?>" id="myForm" onsubmit="return validateForm()" method="post">
            ...
            <div class="mb-5 sm:block md:flex items-center">
                ...
                <div class="w-full lg:w-1/2 flex gap-x-2">
                    <input type="text" id="no_rm_display" value="<?= esc($baris['no_rm'] ?? '') ?>" readonly required
            ...
            <div id="cardPasien" class="<?= empty($baris['no_rm'] ?? '') ? 'hidden ' : '' ?>mb-6 bg-slate-50 border border-slate-200 rounded-xl p-5 dark:bg-slate-800 dark:border-slate-700 shadow-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-y-3 gap-x-6 text-sm text-slate-700 dark:text-slate-300">
                    <div class="flex justify-between border-b border-slate-100 pb-1 dark:border-slate-700/50">
                        ...
                        <span id="card_nm_pasien" class="font-semibold text-slate-800 dark:text-white">
                            <?= esc($data_pasien['nama'] ?? '-') ?>
                        </span>
                    </div>
                    <div class="flex justify-between border-b border-slate-100 pb-1 dark:border-slate-700/50">
                        <span class="text-slate-400">NIK:</span>
                        <span id="card_nik" class="font-semibold text-slate-800 dark:text-white">
                            <?= esc($data_pasien['nik'] ?? '-') ?>
                        </span>
                    </div>
                    <div class="flex justify-between border-b border-slate-100 pb-1 dark:border-slate-700/50">
                        <span class="text-slate-400">Tanggal Lahir:</span>
                        <span id="card_tgl_lahir" class="font-semibold text-slate-800 dark:text-white">
                            <?= esc($data_pasien['tanggal_lahir'] ?? '-') ?>
                        </span>
                    </div>
                </div>
            </div>
